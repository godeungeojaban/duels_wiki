...
const state = { index:null, localDocuments:[], current:null, currentPath:'', currentLocalId:null, localMode:false, editing:false, savedRange:null, componentInsertionRange:null, footnoteInsertionRange:null, footnoteInsertionEditable:null, duelsInsertionRange:null, duelsInsertionEditable:null, lastEditable:null, selectedImage:null, selectedComponent:null, descriptionClickArmed:null, activeRibbon:'home', config:null };
...
async function refreshIndex(){ const [idx,local]=await Promise.all([api('/api/index'),api('/api/local-documents').catch(()=>({documents:[]}))]); state.index=idx; if(state.index)state.index.categories=sortedCategories(state.index.categories); state.localDocuments=Array.isArray(local?.documents)?local.documents:[]; renderTree(); refreshInternalLinkDecorations(document); }
function renderTree(){ const tree=$('#tree'); if(!state.index){tree.innerHTML='<div class="loading">불러오는 중…</div>';return}
  ...
  html+=`<div class="tree-cat tree-local-category"><div class="tree-cat-title tree-local-title">로컬 문서</div>${state.localDocuments.length?state.localDocuments.map(d=>`<button class="tree-doc tree-local-doc" data-local-id="${escapeHtml(d.id)}">${escapeHtml(d.title)}</button>`).join(''):'<div class="tree-local-empty">저장된 로컬 문서 없음</div>'}</div>`;
  tree.innerHTML=html;
  ...
  $$('[data-local-id]',tree).forEach(b=>b.addEventListener('click',async()=>{try{await openLocalDocument(b.dataset.localId);closeMobileSidebar()}catch(e){showStatus(e.message,true)}}));
}
...
function internalLinkMeta(raw,a=null){
  if(!raw?.startsWith('wiki:/'))return null;
  const x=raw.slice(6),parts=x.replace(/^\//,'').split('/').filter(Boolean).map(v=>{try{return decodeURIComponent(v)}catch{return v}});
  if(!parts.length)return {exists:true,category:'Duels Wiki',document:'전체'};
  const cat=state.index?.categories?.find(c=>c.slug===parts[0]);
  if(parts.length===1){return {exists:!!cat?.hasInfo,category:cat?.name||a?.dataset?.wikiCategoryName||parts[0],document:'정보'};}
  const doc=cat?.documents?.find(d=>d.slug===parts[1]);
  return {exists:!!doc,category:cat?.name||a?.dataset?.wikiCategoryName||parts[0],document:doc?.title||a?.dataset?.wikiDocumentName||parts[1]};
}
function decorateInternalLink(a){
  const raw=a?.dataset?.wikiLink||a?.getAttribute?.('href')||'';if(!raw.startsWith('wiki:/'))return;
  a.dataset.wikiLink=raw;const m=internalLinkMeta(raw,a);if(!m)return;
  a.title=`${m.category} / ${m.document}`;
  if(state.index)a.classList.toggle('missing-wiki-link',!m.exists);else a.classList.remove('missing-wiki-link');
}
function refreshInternalLinkDecorations(root=document){$$('a[data-wiki-link],a[href^="wiki:/"]',root).forEach(decorateInternalLink)}
function viewHtml(html,{expandInlineImages=true}={}){ const t=document.createElement('template'); t.innerHTML=html||''; $$('img',t.content).forEach(img=>{const src=img.getAttribute('src')||'';img.setAttribute('src',mediaSrc(src));}); $$('a',t.content).forEach(a=>{const h=a.getAttribute('href')||''; if(h.startsWith('wiki:/')){a.dataset.wikiLink=h;decorateInternalLink(a);}}); upgradeDuelsComponents(t.content); if(expandInlineImages)expandInlineImageSyntax(t.content); if(!state.editing&&window.DuelsReference)window.DuelsReference.expand(t.content); return t.innerHTML; }
...
function bindWikiLinks(root){ bindFootnotePreview(root);refreshInternalLinkDecorations(root); $$('a[data-wiki-link]',root).forEach(a=>a.addEventListener('click',e=>{e.preventDefault(); const x=a.dataset.wikiLink.slice(6); if(!x||x==='/'){location.hash='#/';return} const p=x.replace(/^\//,'').split('/').map(decodeURIComponent); navigate(p[0],p[1]||null);})); $$('[data-section-anchor]',root).forEach(a=>a.addEventListener('click',e=>{e.preventDefault();document.getElementById(a.dataset.sectionAnchor)?.scrollIntoView({behavior:'smooth',block:'start'});})); }

async function renderRoute(){ state.editing=false; state.localMode=false; state.currentLocalId=null; hideToolbar(); state.selectedImage=null; updateOverlay(); const route=parseRoute(); if(!route){await renderHome();return} try{const r=await api(`/api/document?category=${encodeURIComponent(route.category)}${route.doc?`&doc=${encodeURIComponent(route.doc)}`:''}`); state.current=r.document; state.currentPath=r.path; renderDocument(r.document,route);}catch(e){$('#viewPage').innerHTML=`<div class="wiki-card"><h2>문서를 불러올 수 없습니다.</h2><p>${escapeHtml(e.message)}</p></div>`;} }
async function renderHome(){ state.localMode=false;state.currentLocalId=null; try{const r=await api('/api/root'); state.current=r.document; state.currentPath=r.path; renderRootDocument(r.document);}catch(e){state.current=null;state.currentPath='';$('#viewPage').innerHTML=`<div class="wiki-card"><h2>Duels Wiki</h2><p>${escapeHtml(e.message)}</p></div>`;$('#editPage').classList.add('hidden');$('#viewPage').classList.remove('hidden');} }
function renderRootDocument(doc){ hideToolbar();const c=normalizeContent(doc.content); const self='#/'; $('#viewPage').innerHTML=`<div class="wiki-card"><div class="doc-head"><div><h1>${displayTitleHtml(c.titleHtml,doc.title||'Duels Wiki')}</h1></div><div class="doc-actions"><button id="editBtn">문서 편집</button><button id="localSaveBtn">로컬 저장</button><button id="historyBtn">문서 역사</button></div></div><div class="intro wiki-body">${viewHtml(c.introHtml)}</div>${c.sections.length?`<nav class="toc"><div class="toc-title">목차</div>${renderToc(c.sections)}</nav>`:''}${renderSections(c.sections,self)}${renderFootnotes(c)}</div>`; $('#editPage').classList.add('hidden');$('#viewPage').classList.remove('hidden');syncFootnoteRefs($('#viewPage'),c);bindWikiLinks($('#viewPage'));window.DuelsReference?.scheduleResolveRoot($('#viewPage'));$('#editBtn').onclick=()=>startRootEdit(doc);$('#localSaveBtn').onclick=()=>saveViewedDocumentToLocal(doc,{kind:'github-root',path:state.currentPath});$('#historyBtn').onclick=()=>showHistory(); }
function renderDocument(doc,route){ hideToolbar();const c=normalizeContent(doc.content); const self=routeFor(route.category,route.doc); const canDuplicate=doc.kind==='document'; $('#viewPage').innerHTML=`<div class="wiki-card"><div class="doc-head"><div><h1>${displayTitleHtml(c.titleHtml,doc.title)}</h1></div><div class="doc-actions"><button id="editBtn">문서 편집</button>${canDuplicate?'<button id="duplicateBtn">문서 복제</button>':''}<button id="localSaveBtn">로컬 저장</button><button id="historyBtn">문서 역사</button></div></div><div class="intro wiki-body">${viewHtml(c.introHtml)}</div>${c.sections.length?`<nav class="toc"><div class="toc-title">목차</div>${renderToc(c.sections)}</nav>`:''}${renderSections(c.sections,self)}${renderFootnotes(c)}</div>`; $('#editPage').classList.add('hidden');$('#viewPage').classList.remove('hidden');syncFootnoteRefs($('#viewPage'),c);bindWikiLinks($('#viewPage'));window.DuelsReference?.scheduleResolveRoot($('#viewPage'));$('#editBtn').onclick=()=>startEdit(doc,route);if(canDuplicate)$('#duplicateBtn').onclick=()=>duplicateDocumentModal(doc,route);$('#localSaveBtn').onclick=()=>saveViewedDocumentToLocal(doc,{kind:'github-document',path:state.currentPath,category:route.category,doc:route.doc||null});$('#historyBtn').onclick=()=>showHistory(); }
...
async function saveViewedDocumentToLocal(doc,source={}){
  try{
    const r=await api('/api/local-document',{method:'POST',body:JSON.stringify({document:doc,source})});
    showStatus(`“${r.document.title}”을 로컬 JSON으로 저장했습니다.`);
  }catch(e){showStatus(e.message,true)}
}

function renderLocalDocument(doc,localId){
  state.editing=false;document.body.classList.remove('editing-mode');state.localMode=true;state.currentLocalId=localId;state.current=doc;state.currentPath=`local:${localId}`;hideToolbar();
  const c=normalizeContent(doc.content);const self='#/';
  $('#viewPage').innerHTML=`<div class="wiki-card local-document-card"><div class="doc-head"><div><div class="local-doc-badge">LOCAL JSON</div><h1>${displayTitleHtml(c.titleHtml,doc.title||'로컬 문서')}</h1></div><div class="doc-actions"><button id="editBtn">문서 편집</button></div></div><div class="intro wiki-body">${viewHtml(c.introHtml)}</div>${c.sections.length?`<nav class="toc"><div class="toc-title">목차</div>${renderToc(c.sections)}</nav>`:''}${renderSections(c.sections,self)}${renderFootnotes(c)}</div>`;
  $('#editPage').classList.add('hidden');$('#viewPage').classList.remove('hidden');syncFootnoteRefs($('#viewPage'),c);bindWikiLinks($('#viewPage'));window.DuelsReference?.scheduleResolveRoot($('#viewPage'));
  $('#editBtn').onclick=()=>startLocalEdit(doc,localId);
}

async function openLocalDocument(localId){
  try{const r=await api(`/api/local-document?id=${encodeURIComponent(localId)}`);closeModal();renderLocalDocument(r.document,localId)}catch(e){showStatus(e.message,true)}
}
...
function cancelCurrentEdit(){
  ...
  state.editing=false;hideToolbar();clearObjectSelection(false);
  if(state.localMode){renderLocalDocument(state.current,state.currentLocalId);return}
  const route=parseRoute();
  if(route)renderDocument(state.current,route);else renderRootDocument(state.current);
  showStatus('편집을 취소했습니다.');
}
function startLocalEdit(doc,localId){
  state.editing=true;state.localMode=true;state.currentLocalId=localId;editingContent=normalizeContent(doc.content);
  $('#viewPage').classList.add('hidden');$('#editPage').classList.remove('hidden');document.body.classList.add('editing-mode');
  $('#editPage').innerHTML=`<div class="editor-card local-editor-card"><div class="edit-meta"><span class="local-edit-label">로컬 JSON 문서</span></div><div class="title-block"><div id="docTitle" class="title-block-title inline-title-editable" contenteditable="true"></div><div id="introEditor" class="editable rich" contenteditable="true"></div></div><div id="sectionTree"></div><div id="footnoteBlockHost"></div></div><div class="editor-bottom-bar"><div class="editor-bottom-bar-inner"><button id="cancelEditBtn">편집 취소</button><button id="deleteBtn" class="danger">문서 삭제</button><button id="saveBtn" class="primary">로컬 저장</button></div></div>`;
  ...
  renderSectionTree();refreshFootnoteBlock();showToolbar();$('#cancelEditBtn').onclick=cancelCurrentEdit;$('#saveBtn').onclick=()=>saveLocalCurrent(localId);$('#deleteBtn').onclick=async()=>{if(!localId){if(confirm('아직 저장되지 않은 로컬 문서를 닫을까요?')){state.editing=false;state.localMode=false;hideToolbar();document.body.classList.remove('editing-mode');location.hash='#/';await renderRoute()}return}if(!confirm('이 로컬 문서를 삭제할까요?'))return;try{await api(`/api/local-document?id=${encodeURIComponent(localId)}`,{method:'DELETE'});state.editing=false;state.localMode=false;state.current=null;state.currentLocalId=null;hideToolbar();document.body.classList.remove('editing-mode');await refreshIndex();location.hash='#/';await renderRoute();showStatus('로컬 문서를 삭제했습니다.')}catch(e){showStatus(e.message,true)}};
}

async function saveLocalCurrent(localId){
  try{
    editingContent.introHtml=storageHtml($('#introEditor'));editingContent.titleHtml=storageInlineHtml($('#docTitle'));
    $$('.section-edit').forEach(n=>{const sec=findSection(editingContent.sections,n.dataset.sectionId);if(sec){sec.contentHtml=storageHtml($('.section-edit-body',n));const t=$('.section-title-input',n);if(t){sec.titleHtml=storageInlineHtml(t);sec.title=plainTitleFromEditable(t)||'제목 없음'}}});
    $$('.footnote-edit-row').forEach(n=>{const f=(editingContent.footnotes||[]).find(x=>x.id===n.dataset.footnoteId);if(f)f.contentHtml=storageHtml($('.footnote-edit-body',n));});
    ...
    state.current=r.document;const savedLocalId=String(r.document?._local?.id||localId||'');state.currentLocalId=savedLocalId;showStatus('로컬 JSON 문서를 저장했습니다.');renderLocalDocument(r.document,savedLocalId);void refreshIndex().catch(e=>console.warn('로컬 저장 후 목록 갱신 실패',e));
  }catch(e){showStatus(e.message,true)}
}
...
function startRootEdit(doc){ state.editing=true; editingContent=normalizeContent(doc.content); $('#viewPage').classList.add('hidden');$('#editPage').classList.remove('hidden');document.body.classList.add('editing-mode');$('#editPage').innerHTML=`<div class="editor-card"><div class="title-block"><div id="docTitle" class="title-block-title inline-title-editable" contenteditable="true"></div><div id="introEditor" class="editable rich" contenteditable="true"></div></div><div id="sectionTree"></div><div id="footnoteBlockHost"></div></div><div class="editor-bottom-bar"><div class="editor-bottom-bar-inner"><span class="muted">루트 문서는 삭제할 수 없습니다.</span><button id="cancelEditBtn">편집 취소</button><button id="saveBtn" class="primary">문서 저장</button></div></div>`;const title=$('#docTitle');title.innerHTML=displayTitleHtml(editingContent.titleHtml,doc.title||'Duels Wiki');bindInlineFootnoteTarget(title,()=>editingContent.titleHtml=storageInlineHtml(title)); const intro=$('#introEditor'); intro.innerHTML=viewHtml(editingContent.introHtml,{expandInlineImages:false});bindEditable(intro);intro.addEventListener('input',()=>editingContent.introHtml=storageHtml(intro));renderSectionTree();refreshFootnoteBlock();showToolbar();$('#cancelEditBtn').onclick=cancelCurrentEdit;$('#saveBtn').onclick=saveRootCurrent; }
async function saveRootCurrent(){ try{ editingContent.introHtml=storageHtml($('#introEditor')); $$('.section-edit').forEach(n=>{const s=findSection(editingContent.sections,n.dataset.sectionId);if(s){s.contentHtml=storageHtml($('.section-edit-body',n));const t=$('.section-title-input',n);if(t){s.titleHtml=storageInlineHtml(t);s.title=plainTitleFromEditable(t)||'제목 없음'}}}); $$('.footnote-edit-row').forEach(n=>{const f=(editingContent.footnotes||[]).find(x=>x.id===n.dataset.footnoteId);if(f)f.contentHtml=storageHtml($('.footnote-edit-body',n));}); editingContent.titleHtml=storageInlineHtml($('#docTitle'));const rootTitle=plainTitleFromEditable($('#docTitle'))||'Duels Wiki'; const r=await api('/api/root/update',{method:'POST',body:JSON.stringify({title:rootTitle,content:editingContent})}); state.editing=false;state.current=r.document;state.currentPath=`${state.config?.data_root||'wiki'}/_root.json`;history.replaceState(null,'','#/');renderRootDocument(r.document);showStatus('Duels Wiki 문서를 저장했습니다. GitHub Pages는 Actions 완료 후 갱신됩니다.');void refreshIndex().catch(e=>console.warn('저장 후 사이드바 갱신 실패',e)); }catch(e){showStatus(e.message,true)} }

function startEdit(doc,route){ state.editing=true; editingContent=normalizeContent(doc.content); const cats=state.index.categories; const isInfo=doc.kind==='category-info'; $('#viewPage').classList.add('hidden');$('#editPage').classList.remove('hidden');document.body.classList.add('editing-mode');$('#editPage').innerHTML=`<div class="editor-card"><div class="edit-meta"><select id="categorySelect" ${isInfo?'disabled':''}>${cats.map(c=>`<option value="${escapeHtml(c.slug)}" ${c.slug===route.category?'selected':''}>${escapeHtml(c.name)}</option>`).join('')}</select></div><div class="title-block"><div id="docTitle" class="title-block-title inline-title-editable ${isInfo?'disabled-title':''}" contenteditable="${isInfo?'false':'true'}"></div><div id="introEditor" class="editable rich" contenteditable="true"></div></div><div id="sectionTree"></div><div id="footnoteBlockHost"></div></div><div class="editor-bottom-bar"><div class="editor-bottom-bar-inner"><button id="cancelEditBtn">편집 취소</button><button id="deleteBtn" class="danger">${isInfo?'카테고리 삭제':'문서 삭제'}</button><button id="saveBtn" class="primary">문서 저장</button></div></div>`;const title=$('#docTitle');title.innerHTML=displayTitleHtml(editingContent.titleHtml,doc.title);if(!isInfo)bindInlineFootnoteTarget(title,()=>editingContent.titleHtml=storageInlineHtml(title)); const intro=$('#introEditor'); intro.innerHTML=viewHtml(editingContent.introHtml,{expandInlineImages:false});bindEditable(intro);intro.addEventListener('input',()=>editingContent.introHtml=storageHtml(intro));renderSectionTree();refreshFootnoteBlock();showToolbar();$('#cancelEditBtn').onclick=cancelCurrentEdit;$('#saveBtn').onclick=()=>saveCurrent(route,isInfo);$('#deleteBtn').onclick=()=>deleteCurrent(route,isInfo); }
async function saveCurrent(route,isInfo){ try{ editingContent.introHtml=storageHtml($('#introEditor'));if(!isInfo)editingContent.titleHtml=storageInlineHtml($('#docTitle')); $$('.section-edit').forEach(n=>{const s=findSection(editingContent.sections,n.dataset.sectionId);if(s){s.contentHtml=storageHtml($('.section-edit-body',n));const t=$('.section-title-input',n);if(t){s.titleHtml=storageInlineHtml(t);s.title=plainTitleFromEditable(t)||'제목 없음'}}}); $$('.footnote-edit-row').forEach(n=>{const f=(editingContent.footnotes||[]).find(x=>x.id===n.dataset.footnoteId);if(f)f.contentHtml=storageHtml($('.footnote-edit-body',n));});const nextTitle=isInfo?state.current?.title:(plainTitleFromEditable($('#docTitle'))||state.current?.title||'제목 없음'); const nextCat=isInfo?route.category:$('#categorySelect').value;const r=await api('/api/document/update',{method:'POST',body:JSON.stringify({category:route.category,doc:route.doc,title:nextTitle,nextCategory:nextCat,content:editingContent})}); const savedDoc=r.document;const nextDoc=isInfo?null:(savedDoc.slug||safeSlugClient(nextTitle));state.editing=false;state.current=savedDoc;state.currentPath=`${state.config?.data_root||'wiki'}/${nextCat}/${isInfo?'_info':nextDoc}.json`;history.replaceState(null,'',routeFor(nextCat,nextDoc));renderDocument(savedDoc,{category:nextCat,doc:nextDoc});showStatus('저장했습니다. GitHub Pages는 Actions 완료 후 갱신됩니다.');void refreshIndex().catch(e=>console.warn('저장 후 사이드바 갱신 실패',e)); }catch(e){showStatus(e.message,true)} }
...
function handleProtectedDeletion(el,e){
  ...
  if(e.target?.closest?.('td[data-table-cell][data-table-cell-editing="1"],.duels-description-box-body[data-description-body-editing="1"]'))return false;
...
function bindEditable(el){
  ...
  el.addEventListener('click',e=>{
    ...
    if(component&&el.contains(component)){
      ...
      const descriptionBody=e.target.closest('.duels-description-box-body[data-description-body-editing="1"]');
      ...
      if(component.dataset.duelsComponent==='description-box'&&descriptionBody){state.lastEditable=descriptionBody;rememberSelection();return}
      e.preventDefault();
      clearImageSelection();
      if(component.dataset.duelsComponent==='table'){
        state.descriptionClickArmed=null;
      ...
      }else if(component.dataset.duelsComponent==='description-box'){
        if(state.descriptionClickArmed===component){state.descriptionClickArmed=null;clearComponentCaretAnchors(el);beginDescriptionBoxBodyEdit(component,e)}
        else{clearComponentSelection();clearInlineTableSelection();clearObjectSelection(true);state.descriptionClickArmed=component;placeComponentSideCaret(el,component)}
      }else if(component.dataset.duelsComponent==='character-card'){
        state.descriptionClickArmed=null;selectComponent(component,true)
      }
      clearComponentCaretAnchors(el);
      if(component.dataset.duelsComponent==='description-box'&&state.descriptionClickArmed===component)placeComponentSideCaret(el,component);
      return;
    }
  ...
  el.addEventListener('dblclick',e=>{
    ...
    if(component&&el.contains(component)){
      ...
      if(component.dataset.duelsComponent==='description-box'){state.descriptionClickArmed=null;beginDescriptionBoxBodyEdit(component,e);return}
      if(component.dataset.duelsComponent==='character-card'){selectComponent(component,true);return}
    }
  });
}
...
function exec(cmd,value=null){
  ...
  restoreSelection();
  const editable=(getSelection()?.anchorNode?.nodeType===1?getSelection()?.anchorNode:getSelection()?.anchorNode?.parentElement)?.closest?.('[data-table-cell-editing="1"],.duels-description-box-body[data-description-body-editing="1"],.editable');
...
function endDescriptionBoxBodyEdit(box=null){
  const roots=box?[box]:$$('[data-duels-component="description-box"]',$('#editPage'));
  roots.forEach(root=>{const body=root?.querySelector?.('.duels-description-box-body[data-description-body-editing="1"]');if(!body)return;body.removeAttribute('data-description-body-editing');body.setAttribute('contenteditable','false');root.classList.remove('description-body-editing')});
}
function clearComponentSelection(){
  endDescriptionBoxBodyEdit();
...
function placeCaretFromPointer(node,e){
  let r=null;
  ...
  if(!r||!node.contains(r.startContainer)){r=document.createRange();r.selectNodeContents(node);r.collapse(false)}
  const sel=getSelection();sel.removeAllRanges();sel.addRange(r);state.savedRange=r.cloneRange();
}
function beginDescriptionBoxBodyEdit(box,e){
  if(!box?.isConnected)return;
  selectComponent(box,true);
  let body=box.querySelector('.duels-description-box-body');
  if(!body){body=document.createElement('div');body.className='duels-description-box-body';body.innerHTML='<br>';box.appendChild(body)}
  box.classList.add('description-body-editing');body.dataset.descriptionBodyEditing='1';body.setAttribute('contenteditable','true');
  body.focus({preventScroll:true});placeCaretFromPointer(body,e);state.lastEditable=body;rememberSelection();
}
...
function updateDescriptionRibbonTools(){const box=selectedDescriptionBox();if(!box)return;const d=componentPayload(box);$('#descriptionRibbonTitle').value=d.title??'';$('#descriptionRibbonColor').value=componentColor(d.color)}
...
function applyDescriptionRibbon(){const box=selectedDescriptionBox();if(!box)return;const current=componentPayload(box);const d={...current,title:$('#descriptionRibbonTitle').value.trim(),color:$('#descriptionRibbonColor').value};renderDescriptionBoxElement(box,d);box.classList.add('selected-duels-component');state.selectedComponent=box;componentInputEvent(box)}
...
function currentBlocks(){
  const sel=getSelection(); if(!sel.rangeCount)return[]; const range=sel.getRangeAt(0); const root=range.commonAncestorContainer.nodeType===1?range.commonAncestorContainer:range.commonAncestorContainer.parentElement; const editable=root?.closest?.('[data-table-cell-editing="1"],.duels-description-box-body[data-description-body-editing="1"],.editable'); if(!editable)return[];
...
function applyInternalLink(href,categoryName,documentName){
  restoreSelection();document.execCommand('createLink',false,href);
  const sel=getSelection();const node=sel?.anchorNode?.nodeType===1?sel.anchorNode:sel?.anchorNode?.parentElement;let a=node?.closest?.('a');
  if(!a||a.getAttribute('href')!==href){a=$$('a',$('#editPage')).reverse().find(x=>x.getAttribute('href')===href)||null}
  if(a){a.dataset.wikiLink=href;if(categoryName)a.dataset.wikiCategoryName=categoryName;if(documentName)a.dataset.wikiDocumentName=documentName;decorateInternalLink(a)}
  closeModal();rememberSelection();
}
function openInternalLink(){
  rememberSelection(); const cats=state.index?.categories||[];
  openModal(`<h2>내부 링크</h2><div class="form-row"><label>카테고리</label><select id="linkCat"><option value="__all__">Duels Wiki (전체)</option>${cats.map(c=>`<option value="${escapeHtml(c.slug)}">${escapeHtml(c.name)}</option>`).join('')}</select></div><div id="linkChoices" class="link-list"></div><div class="internal-link-manual"><div class="form-row"><label>존재하지 않는 문서도 미리 연결</label><input id="linkManualCategory" placeholder="카테고리명"><input id="linkManualDocument" placeholder="문서명"></div><button id="linkManualApply" type="button">이 경로로 링크</button><p class="muted">문서가 아직 없으면 다른 색으로 표시되며, 같은 카테고리/문서명이 생성되면 정상 링크로 바뀝니다.</p></div><div class="modal-actions"><button id="cancelLink">취소</button></div>`);
  const select=$('#linkCat');
  const draw=()=>{const v=select.value,box=$('#linkChoices');if(v==='__all__')box.innerHTML='<button class="link-choice" data-href="wiki:/" data-cat-name="Duels Wiki" data-doc-name="전체">Duels Wiki / 전체</button>';else{const c=cats.find(x=>x.slug===v);box.innerHTML=(c?.hasInfo?`<button class="link-choice" data-href="wiki:/${encodeURIComponent(v)}" data-cat-name="${escapeHtml(c.name)}" data-doc-name="정보">${escapeHtml(c.name)} / 정보</button>`:'')+(c?.documents||[]).map(d=>`<button class="link-choice" data-href="wiki:/${encodeURIComponent(v)}/${encodeURIComponent(d.slug)}" data-cat-name="${escapeHtml(c.name)}" data-doc-name="${escapeHtml(d.title)}">${escapeHtml(d.title)}</button>`).join('');$('#linkManualCategory').value=c?.name||''} $$('.link-choice',box).forEach(b=>b.onclick=()=>applyInternalLink(b.dataset.href,b.dataset.catName,b.dataset.docName))};
  select.onchange=draw;draw();
  $('#linkManualApply').onclick=()=>{const catName=$('#linkManualCategory').value.trim(),docName=$('#linkManualDocument').value.trim();if(!catName||!docName){alert('카테고리명과 문서명을 모두 입력해주세요.');return}const cat=cats.find(c=>c.name===catName||c.slug===catName),catSlug=cat?.slug||safeSlugClient(catName),doc=cat?.documents?.find(d=>d.title===docName||d.slug===docName),docSlug=doc?.slug||safeSlugClient(docName);applyInternalLink(`wiki:/${encodeURIComponent(catSlug)}/${encodeURIComponent(docSlug)}`,cat?.name||catName,doc?.title||docName)};
  $('#cancelLink').onclick=closeModal;
}
...
['descriptionRibbonTitle','descriptionRibbonColor'].forEach(id=>$('#'+id)?.addEventListener('input',applyDescriptionRibbon));
...
document.addEventListener('input',e=>{const body=e.target.closest?.('.duels-description-box-body[data-description-body-editing="1"]');if(!body)return;const box=body.closest('[data-duels-component="description-box"]');if(!box)return;const d=componentPayload(box);d.bodyHtml=body.innerHTML;d.body=body.innerText.replace(/\u00a0/g,' ');setComponentPayload(box,d);componentInputEvent(box);rememberSelection()});
...
$('#newDocumentBtn').onclick=async()=>{
  const cats=sortedCategories(state.index?.categories||[]);
  if(!state.config?.tokenConfigured){showStatus('GitHub 문서를 생성하려면 GitHub Token 인증이 필요합니다.',true);return}
  let externalLocalDocument=null;
  openModal(`<h2>문서 생성</h2>
    <div class="form-row"><label>생성 방식</label><select id="newDocMode"><option value="blank">새 빈 문서</option><option value="local">로컬 JSON에서 생성</option></select></div>
    <div id="newDocLocalRow" class="form-row hidden">
      <label>로컬 JSON 파일</label>
      <button type="button" id="newDocBrowseFile">Windows 파일 탐색기에서 JSON 선택</button>
      <input id="newDocExternalFile" type="file" accept=".json,application/json" class="hidden">
      <div id="newDocExternalInfo" class="muted">선택된 JSON 파일이 없습니다.</div>
    </div>
    <div class="form-row"><label>카테고리</label><select id="newDocCat">${cats.map(c=>`<option value="${escapeHtml(c.slug)}">${escapeHtml(c.name)}</option>`).join('')}</select></div>
    <div class="form-row"><label>문서 제목</label><input id="newDocTitle"></div>
    <p id="newDocHelp" class="muted">빈 문서를 새로 생성합니다.</p>
    <div class="modal-actions"><button id="cancelNewDoc">취소</button><button id="createNewDoc" class="primary">생성</button></div>`);
  const mode=$('#newDocMode'),localRow=$('#newDocLocalRow'),title=$('#newDocTitle'),help=$('#newDocHelp'),externalInput=$('#newDocExternalFile'),externalInfo=$('#newDocExternalInfo');
  const parseExternalJson=async file=>{
    const text=await file.text();let data;try{data=JSON.parse(text)}catch{throw new Error('올바른 JSON 문서가 아닙니다.')}
    if(data&&typeof data==='object'&&data.document&&typeof data.document==='object')data=data.document;
    if(!data||typeof data!=='object')throw new Error('문서 JSON 구조가 올바르지 않습니다.');
    if(!data.content||typeof data.content!=='object')throw new Error('Duels Wiki 문서의 content 데이터가 없습니다.');
    externalLocalDocument=data;
    title.value=String(data.title||file.name.replace(/\.json$/i,'')||'로컬 문서');
    externalInfo.textContent=`선택한 파일: ${file.name}`;
  };
  const syncMode=()=>{
    const isLocal=mode.value==='local';
    localRow.classList.toggle('hidden',!isLocal);
    help.textContent=isLocal?'Windows 파일 탐색기에서 Duels Wiki 로컬 JSON을 선택한 뒤 카테고리를 지정해 GitHub 문서로 생성합니다. 원본 JSON은 변경하지 않습니다.':'빈 문서를 새로 생성합니다.';
    if(!isLocal){externalLocalDocument=null;externalInput.value='';externalInfo.textContent='선택된 JSON 파일이 없습니다.';title.value='';}
  };
  $('#newDocBrowseFile').onclick=()=>externalInput.click();
...
async function createLocalDocumentFromSidebar(){const title=String(prompt('새 로컬 문서의 문서명을 입력하세요.')||'').trim();if(!title)return;try{const content={type:'wiki-sections-v3',titleHtml:escapeHtml(title),introHtml:'<p></p>',sections:[{id:uid('sec'),title:'개요',contentHtml:'<p></p>',children:[]}],footnotes:[]};const draft={id:`local_${uid('doc')}`,kind:'local-document',title,slug:safeSlugClient(title),content};const r=await api('/api/local-document',{method:'POST',body:JSON.stringify({document:draft})});const localId=String(r.document?._local?.id||'');await refreshIndex();state.current=r.document;state.currentLocalId=localId;state.localMode=true;startLocalEdit(r.document,localId);showStatus(`“${title}” 로컬 문서를 생성했습니다.`)}catch(e){alert(e.message)}}
$('#newLocalDocumentBtn').onclick=createLocalDocumentFromSidebar;
$('#openLocalJsonBtn').onclick=()=>$('#openLocalJsonFile').click();
$('#openLocalJsonFile').onchange=async e=>{const f=e.target.files?.[0];if(!f)return;try{const text=await f.text();let data=JSON.parse(text);if(data&&typeof data==='object'&&data.document&&typeof data.document==='object')data=data.document;if(!data||typeof data!=='object'||!data.content||typeof data.content!=='object')throw new Error('Duels Wiki 문서 JSON 구조가 올바르지 않습니다.');const title=String(data.title||f.name.replace(/\.json$/i,'')||'로컬 문서').trim()||'로컬 문서';data={...data,title,kind:'local-document'};state.current=data;state.currentLocalId=null;state.localMode=true;startLocalEdit(data,null)}catch(err){alert(err.message)}e.target.value=''};
...
$('.brand').addEventListener('click',e=>{if(state.localMode&&location.hash==='#/'){e.preventDefault();renderRoute()}});
...
(async function init(){try{state.config=await api('/api/config');void syncDisplayedVersion();if(!state.config.tokenConfigured)showStatus('GitHub Token 없이도 로컬 문서 생성·불러오기·편집·저장이 가능합니다. GitHub 편집/업로드에는 Token이 필요합니다.');await renderRoute();const loadIndex=()=>refreshIndex().catch(e=>{console.warn('사이드바 인덱스 로드 실패',e);showStatus('사이드바 문서 목록을 불러오지 못했습니다.',true)});if('requestIdleCallback' in window)requestIdleCallback(loadIndex,{timeout:700});else setTimeout(loadIndex,0);}catch(e){showStatus(e.message,true);$('#viewPage').innerHTML=`<div class="wiki-card"><h2>초기화 실패</h2><p>${escapeHtml(e.message)}</p><p>우측 상단 GitHub 설정에서 저장소와 토큰을 확인해주세요.</p></div>`}})();
