...
Duels Wiki 3.59는 Python 표준 라이브러리 기반 로컬 편집기 + GitHub 저장소 + GitHub Pages 정적 열람 사이트 구조다. Node.js/Next.js/npm은 사용하지 않는다.
...
`이미지`는 글 흐름 안의 인라인 이미지로 렌더링하고, 그 외 값은 일반 텍스트로 렌더링한다. `체력`, `스태미나`, `이동속도`, `난이도`, `공격력`, `방어력`, `설명` 등은 편의를 위한 별칭이다. 원본 객체에 존재하는 실제 필드 키도 그대로 두 번째 인수에 넣어 조회할 수 있다. `필드목록`은 인식된 원본 필드 이름을, `스킬목록`은 인식된 스킬 이름을 보여준다.
