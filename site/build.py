...
def build():
    ...
    idx=['<main class="page index"><h1>Duels Wiki</h1>']
    for c,docs in allcats:
        if c.get('hasInfo'):idx.append(f'<h2><a href="{quote(c["slug"])}/index.html">{esc(c["name"])}</a></h2>')
        else:idx.append(f'<h2>{esc(c["name"])}</h2>')
        idx.append('<ul>')
        for d in docs:idx.append(f'<li><a href="{quote(c["slug"])}/{quote(d["slug"])}/index.html">{esc(d["title"])}</a></li>')
        idx.append('</ul>')
    idx.append('</main>')
    (OUT/'index.html').write_text(shell('Duels Wiki',''.join(idx),'index.html'),'utf-8')
    for c,docs in allcats:
        if c.get('hasInfo'):
